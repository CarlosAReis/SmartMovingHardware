...
char msg2[100];
long count=0;
long contadortimeout =0;
long contadormqtttimeout =0;
...
// Verifica se já passou 180000 milisegundos
  if((millis() - millisTarefa1) > 180000){
    // envia o keep alive
    if (client.connected()){
    String str1 = "{\"sensor_id\":\"" + DispID + "\"}";
    str1.toCharArray(msg2,100);
    client.publish(root_topic_publish01,msg2);
    delay(30);
...
void setup_wifi(){
  ...
  //wifiMulti.run(WiFi_timeout);
 // WiFi.begin(ssid1, password);
  contadortimeout = 0;
  
  while (wifiMulti.run(WiFi_timeout)!= WL_CONNECTED and contadortimeout < 10) {
    delay(500);
    Serial.print("...");
    contadortimeout = contadortimeout +1;
  }

  if(contadortimeout < 10){
    Serial.println("");
...
void reconnect() {
  contadormqtttimeout =0;
  while (!client.connected()and contadormqtttimeout <10 ) {
   ...
   // clientId += String(random(0xffff), HEX);
    ...
    if (client.connect(clientId.c_str())) {
      ...
      delay(5000);
      contadormqtttimeout = contadormqtttimeout +1;
    }
